...
window.onload = function () {
    ...
    // alert(session.getObject("loginuser"));
	if(session.getObject("loginuser") != null){
        loggedUserName = session.getObject("loginuser").
            loginusername;
        ...
        if(loggedUser.employeeId != undefined){
            if(loggedUser.employeeId.photo != null) {
                ...
                profileImage.src = '/image/user.jpg';
            }
    ...
    if(session.getObject("activeuser").employeeId.id !=1) {
        ...
        if(session.getObject("activeuser").employeeId.designationId.id ==4){
            var divelement =  document.getElementsByClassName("CDRESERVATION");
            if(divelement.length != 0)
                for(var i=0;i<divelement.length;i++){
                    divelement[i].style.display = "none";
                }
