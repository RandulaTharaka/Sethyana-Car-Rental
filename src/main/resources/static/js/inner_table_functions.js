...
function tableForInnerForm(parentId, id, metadata, hasBtn = true) {
    ...
    // thead.classList.add("bg-secondary-light", "text-white");
...
function fillInnerTable(tableId, obs, modifyFunction = null, deleteFunction = null, editbuttonvis = true ) {
    ...
    for (var i = 0; i < obs.length; i++) {
        ...
        if (hasButton) {
            ...
            buttonInnerDelete.classList.add("btn-sm", "btn-outline-danger", "py-0", "text-dark", "mb-resp-s",);
            buttonInnerDelete.setAttribute('title','Delete');
            buttonInnerDelete.setAttribute('data-placement','top');
            buttonInnerDelete.innerHTML = '<i class="fas fa-trash"></i> ';
            buttonInnerDelete.onclick = function () {
                idx = window.parseInt(this.parentNode.parentNode.firstChild.innerHTML); // idx: object of the list, idx: relevant row
